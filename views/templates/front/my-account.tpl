...
{if isset($reload) && $reload == 1}
	{literal}
		<script type="text/javascript">
			document.location.href = "{/literal}{$link->getModuleLink('refpro', 'myaccount')|escape:'quotes':'UTF-8'}{literal}";
		</script>
	{/literal}
{/if}
...
{if isset($ifGreateThen16) && !$ifGreateThen16}
    {capture name=path}
        <a href="{$link->getPageLink('my-account')|escape:'htmlall':'UTF-8'}">{l s='My account' mod='refpro'}</a>
        <span class="navigation-pipe">{$navigationPipe|escape:'htmlall':'UTF-8'}</span>{l s='Affiliate program' mod='refpro'}
    {/capture}
    {include file="$tpl_dir./breadcrumb.tpl"}
{/if}
<h1 class="block_header corners">{l s='Affiliate program' mod='refpro'}</h1>
...
<div id="tabs" class="tab_container">
	<ul class="tabs">
		...
		{if isset($is_sponsor_tmp) && $is_sponsor_tmp}
			...
			{if $p_systems && $withdrawal_allowed}
	...
	<div class="tabs_content">
		<div class="tab" id="tabs-1">
			{if isset($is_sponsor_tmp) && $is_sponsor_tmp}
                ...
                <p><a href="{$link_num|escape:'htmlall':'UTF-8'}" target=new>{$link_num|escape:'htmlall':'UTF-8'}</a></p>
...
{else}
	...
	<span class="money">{convertPriceWithCurrency price=$money currency=$currency_default}</span>
                    {if $withdrawal_allowed && isset($not_set)}
...
{/if}
                ...
                <table class="table">
					{if Referral::getSettings('zero_level_rewards') && isset($rate_0) && $rate_0}
						<tr>
							<td><b>{l s='My cashback:' mod='refpro'}</b></td>
							<td><b>{if isset($rate_0) && $rate_0}{$rate_0|escape:'htmlall':'UTF-8'}{else}0{/if}</b> %</td>
						</tr>
					{/if}
					
                    {if isset($rate_1) && $rate_1}					
                    <tr>
                        <td>{l s='The first level:' mod='refpro'}</td>
                        <td><b>{$rate_1|escape:'htmlall':'UTF-8'}</b> %</td>							   
                    </tr>
                    {/if}
				...
				{if $is_need_completed_orders}
					...
					{if isset($rules)}
						<p>
												...
												<label for="ref_agree" class="ref_lbl" style="color: #000000; margin-left: 5px;">{l s='I have read and agree to the' mod='refpro'} <a href="{$rules|escape:'htmlall':'UTF-8'}" id="ref_agree" class="iframe">"{l s='Terms of Service' mod='refpro'}"</a>.</label>
						</p>
					{else}
		...
		{if isset($is_sponsor_tmp) && $is_sponsor_tmp}
			<div class="tab" id="tabs-2">
				...
				{if isset($subrefs3) && !empty($subrefs3) && $rate_3}
					<br />
				...
				{if isset($subrefs4) && !empty($subrefs4) && $rate_4}
					<br />
				...
				{if isset($subrefs5) && !empty($subrefs5) && $rate_5}
					<br />
                ...
                {if isset($subrefs6) && !empty($subrefs6) && $rate_6}
					<br />
                ...
                {if isset($subrefs7) && !empty($subrefs7) && $rate_7}
					<br />
				...
				{if isset($subrefs8) && !empty($subrefs8) && $rate_8}
					<br />
				...
				{if isset($subrefs9) && !empty($subrefs9) && $rate_9}
					<br />
					...
					<table class="table">
						{foreach from=$banners item=banner}
							<tr>
								...
								<td><textarea style="width:100%;height:60px;" onClick="this.select();" title="{l s='Click and use Ctrl+C to copy' mod='refpro'}" readonly>&lt;a href=&quot;{$banner.url|escape:'htmlall':'UTF-8'}&quot; target=&quot;new&quot;&gt;&lt;img src=&quot;{$banner.b|escape:'htmlall':'UTF-8'}&quot;&gt;&lt;/a&gt;</textarea></td>
							</tr>
						{/foreach}
					</table>
				...
				<div class="tab" id="tabs-4">
					<table class="table">
						...
						{foreach from=$payments item=payment}
							<tr>
								<td>{$payment.ts|escape:'quotes':'UTF-8'}</td>
								<td>{convertPriceWithCurrency price=$payment.amount currency=$currency_default}</td>
							</tr>
						{/foreach}
						</tbody>
					</table>
...
{if $banner_url}
<a href="{$banner_url|escape:'htmlall':'UTF-8'}" class="refbanner" target="_blank"><img src="{$module_img_dir|escape:'htmlall':'UTF-8'}{$banner_img|escape:'quotes':'UTF-8'}" alt="" title="" /></a>
{/if}

{if isset($ifGreateThen16) && $ifGreateThen16}
	<ul class="footer_links clearfix" style="margin-top: 20px;">
		<li><a class="btn btn-default button-small" href="{$link->getPageLink('my-account')|escape:'quotes':'UTF-8'}" title="{l s='Back to Your Account' mod='refpro'}"><span><i class="icon-chevron-left"></i>{l s='Back to Your Account' mod='refpro'}</span></a></li>
		<li><a class="btn btn-default button-small" href="{$base_dir|escape:'htmlall':'UTF-8'}" title="{l s='Home' mod='refpro'}"><span><i class="icon-chevron-left"></i>{l s='Home' mod='refpro'}</span></a></li>
	</ul>
{else}
	<ul class="footer_links">
		<li><a href="{$link->getPageLink('my-account')|escape:'quotes':'UTF-8'}"><img src="{$ref_pro_img_dir|escape:'htmlall':'UTF-8'}icon/my-account.gif" alt="" class="icon" /></a><a href="{$link->getPageLink('my-account')|escape:'quotes':'UTF-8'}">{l s='Back to Your Account' mod='refpro'}</a></li>
		<li><a href="{$base_dir|escape:'htmlall':'UTF-8'}"><img src="{$ref_pro_img_dir|escape:'htmlall':'UTF-8'}icon/home.gif" alt="" class="icon" /></a><a href="{$base_dir|escape:'htmlall':'UTF-8'}">{l s='Home' mod='refpro'}</a></li>
	</ul>
{/if}
{else}
