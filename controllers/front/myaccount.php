...
require_once dirname(__FILE__).'/../../lib/QRCode.php';
require_once dirname(__FILE__).'/../../lib/QROptions.php';
...
$this->context->smarty->assign(array('user_data' => Referral::getUserData($this->context->customer->id)));
		...
		if (!empty($s_goref) && Referral::checkNeedCompletedOrders())
		{
			if (Referral::getUserData($this->context->customer->id))
			    ...
			    $this->module->sendMailNotification($this->context->customer->id);
		}
		else
		{
		...
		if (!empty($result['id']))
		{
			...
			$rate_0 = Referral::getBonusRate($this->context->customer->id_default_group, 0);																	   
			...
			if (Referral::getSettings('levels') > 0) $assign_arr['rate_1'] = $rate_1;																
	...
	protected function generateSvgQrCode($data)
	{
		foreach($this->qrCodeVersionToMaxLength as $version => $maxLength) {
			if (Tools::strlen($data) > $maxLength) {
				continue;
			}
			$qrCode = new QRCode(
				new QROptions(
