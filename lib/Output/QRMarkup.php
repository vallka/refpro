...
require_once __DIR__.'/QROutputAbstract.php';
...
class QRMarkup extends QROutputAbstract{
	...
	protected function svg(){
		...
		$svg = sprintf($this->svgHeader, $this->options->cssClass, isset($this->options->svgViewBoxSize) ? $this->options->svgViewBoxSize : $this->moduleCount)
		       .$this->options->eol
			...
			foreach($matrix as $y => $row){
				...
				foreach($row as $x => $module){
					...
					if($module === $M_TYPE){
						...
						if(isset($row[$x + 1]) ? $row[$x + 1] : false){
							continue;
						}
					}
